...
function montarPreventivo(app, opciones) {
  ...
  app.post(`${base}/equipo`, auth, asinc(async (req, res) => {
    ...
    const equipo = {
      ...
      // Blindaje: aquí solo entra la política. Si llegara el nombre de una
      // empresa o de una persona, el equipo quedaría fuera de todos los filtros
      // y la app no lo volvería a ver nunca.
      responsable: ['oficial', 'externo'].includes(norm(p.responsable).toLowerCase())
  ...
  app.post(`${base}/cambio`, auth, asinc(async (req, res) => {
    ...
    for (const campo of Object.keys(campos)) {
      const nuevo = norm(campos[campo]);
      const anterior = norm(eq[campo]);
...
const CAMPOS_APROBACION = new Set([
  'periodicidad_dias', 'criticidad', 'responsable', 'estado_equipo',
